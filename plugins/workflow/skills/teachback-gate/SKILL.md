...
2. Read `references/output-preferences.md` and apply the configured output language/style.
3. Produce a brief under 12 lines.
4. Ask the operator to explain the change back in exactly three bullets.
5. If the operator's explanation misses a key behavior, risk, or rollback fact, ask one follow-up question.
6. If the explanation is good enough, say `Gate passed` and list the next safe command.
